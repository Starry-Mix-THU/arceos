...
use crate::smoltcp_impl::addr::from_core_ipaddr;
    ...
    fn handle_recv_result(
        ...
        match result {
            Ok((src, meta)) => {
                ...
                Ok((read, into_core_sockaddr(meta.endpoint)))
            }
        ...
        self.recv_impl(|socket| {
            Self::handle_recv_result(buf, socket.recv(), None).and_then(|(len, meta_addr)| {
                if (!is_unspecified(remote_endpoint.addr)
                    && remote_endpoint.addr != from_core_ipaddr(meta_addr.ip()))
